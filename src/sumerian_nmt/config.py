...
- Legacy mT5 NMT (archived in baselines/mt5/)

All paths and configurations for training, evaluation, and inference.

Path Resolution:
- SUMERIAN_HOME environment variable (highest priority)
- Editable install fallback (src/sumerian_nmt/config.py -> 3 levels up)
"""
...
class Paths:
    """Centralized path configuration with robust resolution."""

    # Robust path resolution: Env var > Editable install > Package install
    if os.environ.get("SUMERIAN_HOME"):
        ROOT = Path(os.environ["SUMERIAN_HOME"])
    else:
        # Fallback for editable install / local dev
        # src/sumerian_nmt/config.py -> 3 levels up to repo root
        ROOT = Path(__file__).resolve().parent.parent.parent

    # Core directories
    DATA_DIR = ROOT / "data"
    MODELS_DIR = ROOT / "models_llm"
    CONFIGS_DIR = ROOT / "configs"

    @classmethod
    def validate(cls):
        """Validate critical directories exist and warn if missing."""
        if not cls.ROOT.exists():
            raise RuntimeError(f"SUMERIAN_HOME invalid: {cls.ROOT}")
        if not cls.DATA_DIR.exists():
            print(f"WARNING: Data directory not found at {cls.DATA_DIR}")
    ...
    # LLM training configs (updated path: was configs_llm/)
    CONFIGS_LLM = ROOT / "configs"
    ...
    # ==========================================================================
    # LEGACY PATHS (For backward compatibility with baselines/mt5/)
    # ==========================================================================
    ...
    def validate(cls):
        ...
        if not cls.ROOT.exists():
            raise RuntimeError(
                f"SUMERIAN_HOME invalid or ROOT not found: {cls.ROOT}\n"
                f"Set SUMERIAN_HOME environment variable or run from repository root."
            )

        # Warn about missing data directories (non-fatal)
        if not (cls.ROOT / "data").exists():
            print(f"WARNING: Data directory not found at {cls.ROOT / 'data'}")

        if not cls.FINAL_LLM_DATA.exists():
            print(f"WARNING: Training data not found at {cls.FINAL_LLM_DATA}")
            print("  Run: cd data && unzip -o '*.zip'")
...
class LLMConfig:
    ...
    def format_prompt(cls, instruction: str, input_text: str) -> str:
        ...
        return (
            ...
            f"{cls.SYSTEM_PROMPT}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n"
            f"{instruction}\n\n"
...
# =============================================================================
# LEGACY CONFIGURATION (For mT5 - archived in baselines/)
# =============================================================================
...
# These allow baselines/mt5/ scripts to import from config without modification
class ModelConfigs:
